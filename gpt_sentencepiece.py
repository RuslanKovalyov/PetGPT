...
# encoder with vocab50k
with open('vocab_11706_shakespeare.json', 'r') as fp: # small vocab for test
     vocab = json.load(fp)
...
# ----------------- remap ids ------------------
# 11706 is is max id in vocab, use remap to get it from numbers up to 50257
# create table for remapping, take all "idx": "token" pairs from vocab and make new dict with "50k_idx": "idx"
remap = {}
for i in enumerate(vocab):
    remap[i[1]] = i[0]

# remap les ids-50k to 11k
def remaped_50k_to_11k(idx):
    return list(map(lambda x: remap[str(x)], idx))

# remap les ids-11k to 50k
def remaped_11k_to_50k(idx):
    return list(map(lambda x: int(list(remap.keys())[list(remap.values()).index(x)]), idx))

print('\n\nremaped_ids 50k to 11k: [3, 50187, 6, 11] --> ',remaped_50k_to_11k([3, 50187, 6, 11]))

# remapded encode/decode
encode = lambda s: remaped_50k_to_11k(enc.encode(s)) # + [0] * block_size
decode = lambda idx: enc.decode(remaped_11k_to_50k(idx))
# ---------------- and remap ids ----------------


# output some stats
# print("\nVocabulary: ",  s2i)
print("Vocabulary size: ", vocab_size, "words/tokens")
print(f"settins:\n\
...
print('start the program at', time.strftime("%H:%M:%S", time.localtime(time.time())))
...
try:
    with open('model_sen_piece.pt', 'rb') as f:
...
for iter in range(max_iters):
    ...
    if (iter % eval_interval == 0 or iter == max_iters - 1):# and iter != 0:
        ...
        # save a checkpoint
        try:
            # torch.save({'model': m.state_dict(), 'optimizer': optimizer.state_dict()}, 'model_sen_piece.pt')
